...
/** Beep when 3 seconds remain before work resumes. */
export function playRestEndingCue() {
  const ctx = getCtx();
  if (!ctx) return;
  void ctx.resume();
  beep(ctx, ctx.currentTime, 988, 0.14);
}

/** Beep when 3 seconds remain on work (before rest starts). */
export function playWorkEndingCue() {
  const ctx = getCtx();
  if (!ctx) return;
  void ctx.resume();
  beep(ctx, ctx.currentTime, 880, 0.14);
}
